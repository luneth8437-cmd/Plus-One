...
Where an agent is justified — and shipped: the post-match opening assistant (`ai_services/opening_assistant.py`). It reads both activity cards and profiles (context-assembly step that strips all identity fields to preserve anonymity), generates 2-3 personalized openers each with a stated reason, passes them through a guardrail step (length cap, per-opener moderation, personal-info-probe filter, dedupe), and falls back to a deterministic generator that uses the same gathered context so personalization survives an LLM outage. That task is open-ended, non-blocking, and low-failure-cost — the profile that fits agent-shaped AI. Crucially, it still does not act: suggestions fill the chat input and the human presses Send. The decision rule this illustrates is the point: choose agent-shaped AI by task shape and failure cost, not by resume keywords.
